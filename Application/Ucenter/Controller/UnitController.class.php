...
class UnitController extends CommonController{
    ...
    public function agentlist()
    {
        ...
        //取出会员单位
        $count =M('userinfo')->where('oid='.$oid.' and otype=2 and vertus = 1'.$str)->count();
        ...
        $show = $page->show();
        $ulist=M('userinfo')->where('user1.oid='.$oid.' and user1.otype=2 and user1.vertus = 1'.$str)->field('user1.*')->table('wp_userinfo user1')
        ...
        {
            $customer_uid = array();
            $customer_where['unit_id'] = array('eq',$v['uid']);
            $customer_where['otype'] = array('eq',0);
            $customer = M('userinfo')->where($customer_where)->select();
            foreach($customer as $row_customer)
            {
                $customer_uid[] = $row_customer['uid'];
            }
            $order_where['is_settle'] = array('eq',0);
            $order_where['uid'] = array('in',$customer_uid);
            $v['capital'] = $order->where($order_where)->sum('trade_amount');
            $v['capital'] = $v['capital']?$v['capital']:0;
                        
            $num = M('userinfo')->where('oid='.$v['uid'].' and otype=4 and vertus = 1')->count();
            $v['num'] = $num;
            $accountinfo = M('accountinfo')->where("uid={$v['uid']}")->find();
            $v['balance'] = $accountinfo['balance'];
        }
    ...
    {
        $oid = $_REQUEST['uid'];
        $count =M('userinfo')->where('oid='.$oid.' and otype=1 and vertus = 1')->count();
        // var_dump($count);die;
        $pagecount = 10;
        $page = new \Think\Page($count , $pagecount);
        $page->parameter = $row; //此处的row是数组，为了传递查询条件
        ...
        $page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
        $show = $page->show();
        $ulist=M('userinfo')->where('oid='.$oid.' and otype=1 and vertus = 1')->limit($page->firstRow.','.$page->listRows)->select();
        foreach($ulist as $key=>$val)
        {
            $num = M('userinfo')->where('oid='.$val['uid'].' and otype=0 and vertus = 1')->count();
            $ulist[$key]['num'] = $num;
        }
        $this->assign('ulist',$ulist);
        $this->assign('page',$show);
    ...
    {
        $oid = $_REQUEST['uid'];
        $count =M('userinfo')->where('oid='.$oid.' and otype=0 and vertus = 1')->count();
        // var_dump($count);die;
        $pagecount = 10;
        $page = new \Think\Page($count , $pagecount);
        $page->parameter = $row; //此处的row是数组，为了传递查询条件
        ...
        $page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
        $show = $page->show();
        $ulist=M('userinfo')->where('oid='.$oid.' and otype=0 and vertus = 1')->limit($page->firstRow.','.$page->listRows)->select();
        $this->assign('ulist',$ulist);
        $this->assign('page',$show);
        $this->display();
    }
