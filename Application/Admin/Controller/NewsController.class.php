...
class NewsController extends CommonController {
   ...
   //         		}
			// }
			if($news->create()){
				$news->ntime  = strtotime(trim(I('post.ntime')));
				$news->ncover = $idcover;
   ...
   //          $info   =   $upload->upload();
	  ...
	  //          		}
			...
			$data['ntime'] 	   = strtotime(trim(I('post.ntime'))); 
	...
	public function newsdel(){
		...
		if(IS_POST){
			...
			$wenzhang = $news->where('nid='.$nid)->find();
			$result = $news->where('nid='.$nid)->delete();
			if($result!==FALSE){
				$ms = I('post.ntitle');
				$info = '删除【'.$wenzhang['ntitle'].'】文章';
				$type = 2;
				user_log($info,$type);
	...
	public function newtypedel(){
		...
		$info 	= $newsinfo->where(array('ncategory' => $fid))->find();
		if($info)
		{
			$this->error('请先删除栏目下的内容');
		}
